...
import { resolveSvgUrl, getWheelValueFromInput, getWheelCenter, getWheelHandlePosition, getSvgArcPath, translateWheelAngle, getSliderValueFromInput, getSliderDimensions, getSliderHandlePosition, getSliderGradient, iroColorPickerOptionDefaults, IroColor } from 'iro-core';
...
function IroHandle(props) {
    ...
    return (h("svg", { className: "iro__handle", x: props.x, y: props.y, style: { overflow: 'visible' } },
        url && (h("use", Object.assign({ xlinkHref: resolveSvgUrl(url) }, props.origin))),
...
var IroWheel = /*@__PURE__*/(function (IroComponent) {
    ...
    IroWheel.prototype.render = function render (props) {
        ...
        var center = getWheelCenter(props);
        var cX = center.x;
        var cY = center.y;
        var handlePos = getWheelHandlePosition(props);
        return (h("svg", { className: "iro__wheel", width: width, height: width, style: {
            ...
            h("g", { className: "iro__wheel__hue", "stroke-width": radius, fill: "none" }, HUE_STEPS.map(function (angle) { return (h("path", { key: angle, d: getSvgArcPath(cX, cY, radius / 2, angle, angle + 1.5), stroke: ("hsl(" + (translateWheelAngle(props, angle)) + ", 100%, 50%)") })); })),
            h("circle", { className: "iro__wheel__saturation", cx: cX, cy: cY, r: radius, fill: ("url(" + (resolveSvgUrl('#' + this.uid)) + ")") }),
...
var IroSlider = /*@__PURE__*/(function (IroComponent) {
    ...
    IroSlider.prototype.handleInput = function handleInput (x, y, bounds, type) {
        ...
        var value = getSliderValueFromInput(this.props, x, y, bounds);
        var channel;
    ...
    IroSlider.prototype.render = function render (props) {
        ...
        var handlePos = getSliderHandlePosition(props);
        var gradient = getSliderGradient(props);
        return (h("svg", { className: "iro__slider", width: width, height: height, style: {
            ...
            h("rect", { className: "iro__slider__value", rx: radius, ry: radius, x: props.borderWidth / 2, y: props.borderWidth / 2, width: width - props.borderWidth, height: height - props.borderWidth, "stroke-width": props.borderWidth, stroke: props.borderColor, fill: ("url(" + (resolveSvgUrl('#' + this.uid)) + ")") }),
            h(IroHandle, { r: props.handleRadius, url: props.handleSvg, origin: props.handleOrigin, x: handlePos.x, y: handlePos.y })));
    };
